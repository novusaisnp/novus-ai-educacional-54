...
- **Fronteira de serviço com o ERP**: é **HTTP**, não banco compartilhado — são dois projetos Supabase diferentes (`ixnpotaccbpcbritxlud` aqui, `lrkebsznehpuascgqbri` no ERP). Webhooks assinados por HMAC (`supabase/functions/edu-erp-webhook`), config por-organização em `src/lib/featureFlags.ts` (`getERPConfig`/`setERPConfig`).
...
- **`.upsert(..., { onConflict: '...' })` com lista de colunas que não bate com nenhuma UNIQUE constraint real falha com 400 (42P10)**, e o toast que aparece pro usuário pode ser totalmente genérico ("Tente novamente") escondendo a causa real — só apareceu ao testar o salvamento de verdade pela primeira vez (`chamada.tsx`, `useGrades.ts`, ambos corrigidos; `uploadAvatar` em `src/lib/storage.ts` tem o mesmo bug, ainda não corrigido, ver `docs/STATUS.md` → Backlog). Antes de confiar num `onConflict`, rode `SELECT pg_get_constraintdef(oid) FROM pg_constraint WHERE conrelid = 'public.<tabela>'::regclass` e confirme que a lista de colunas bate exatamente com uma constraint UNIQUE existente.
- **CLI do Supabase só fica logado numa conta por vez — trocar de projeto entre `novus-educacional` e `novusai-erp` exige re-login**: os dois projetos (`ixnpotaccbpcbritxlud` e `lrkebsznehpuascgqbri`) pertencem a contas Supabase diferentes. `supabase login --token <x>` substitui a sessão inteira; não dá pra estar logado nas duas ao mesmo tempo. Sintoma enganoso: depois de trocar de conta, comandos contra o projeto anterior falham com erro de permissão (403), não com "projeto não encontrado" — parece bug de RLS/acesso quando na verdade é só a conta errada. Ver skill `~/.claude/skills/novus-ecosystem-cli/SKILL.md` pro protocolo completo antes de qualquer trabalho cross-repo.
