...
- **Captcha (Turnstile) no login pode ficar num estado "todo mundo travado" sem nenhum commit quebrado**: `src/components/auth/TurnstileWidget.tsx` é auto-gated por `VITE_TURNSTILE_SITE_KEY` — sem essa env var (não existe em `.env.example`, e não estava setada no deploy em 2026-08-11), o widget não renderiza e o login segue sem `captchaToken`, de propósito. O problema é que isso só é seguro enquanto "Captcha protection" também está desligado no Supabase Dashboard (Authentication → Settings/Attack Protection) — se alguém liga essa proteção no Dashboard (não fica em `supabase/config.toml`, nem em migration nenhuma) sem também configurar `VITE_TURNSTILE_SITE_KEY` no deploy, **todo login para** com o erro genérico `captcha protection: request disallowed (no captcha_token found)`. Antes de mexer nisso: confirme no Dashboard se a proteção está ligada e, se estiver, se `VITE_TURNSTILE_SITE_KEY` está de fato setada no ambiente de deploy (Vercel) — as duas pontas (Dashboard + env var) têm que mudar juntas, uma sem a outra quebra o login pra todo mundo.
