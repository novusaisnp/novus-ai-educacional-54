...
- **Supabase CLI**: nesta sessão o CLI local esteve, em algum momento, logado numa conta que só enxerga um projeto "lignumfleet" (`jargkgufnbgsfcwbibwb`) sem relação com este repo — se `supabase projects list`/`link` falhar, cheque a conta logada (`supabase login`) antes de qualquer outra coisa. Ao longo desta sessão o CLI voltou a apontar certo para `ixnpotaccbpcbritxlud` ("Novus Educacional") e as migrations abaixo foram aplicadas com sucesso via `supabase db query --linked -f`.
- **Descoberta importante de segurança/funcionalidade**: as tabelas `assessments`, `attendance`, `consents`, `documents`, `grades`, `interactions` e `subjects` têm RLS **habilitado sem nenhuma política** — ninguém consegue ler nem escrever nelas hoje (SELECT retorna vazio silenciosamente, INSERT/UPDATE/DELETE falham com "violates row-level security policy"). `classes`, `enrollments` e `student_guardians` tinham o mesmo problema e **foram corrigidas nesta sessão** (ver Fase 1 — Funil de Admissão, abaixo). As 7 tabelas restantes continuam quebradas — ver Backlog.
...
- **RLS habilitado sem nenhuma política** em `assessments`, `attendance`, `consents`, `documents`, `grades`, `interactions`, `subjects` — ninguém consegue ler/escrever nessas tabelas hoje no banco real (`classes`/`enrollments`/`student_guardians` tinham o mesmo problema, corrigido na Fase 1 — Funil de Admissão). Descoberto rodando `SELECT c.relname FROM pg_class c WHERE c.relrowsecurity AND NOT EXISTS (SELECT 1 FROM pg_policies p WHERE p.tablename=c.relname)`. Bloqueia silenciosamente qualquer feature que dependa dessas tabelas (frequência, notas, avaliações, GED, LGPD/consents, disciplinas) mesmo que o código do app esteja certo.
...
- Adicionar as políticas RLS que faltam em `assessments`, `attendance`, `consents`, `documents`, `grades`, `interactions`, `subjects` (mesmo padrão já aplicado em `classes`/`enrollments`/`student_guardians` nesta sessão) — hoje essas tabelas são inacessíveis via app.
...
- **Fase 1 — Secretaria Digital**: (Calendário Letivo) ✅ feito — `periods` + `calendar_exceptions`, marcação por dia ou por intervalo, aviso em `chamada.tsx`. (Funil de Admissão) ✅ feito — reserva de vaga → aprovação/rejeição → conversão real em `students`+`guardians`+`enrollments`, responsável persistido também no cadastro direto de aluno. Ainda por fazer: assinatura eletrônica de contrato com gatilho real pra Porta 1 do ERP, GED do aluno (`documents`) com validação por IA (bloqueado hoje por RLS sem política, ver Backlog). **Somar do MVP novo**: recuperação/progressão parcial/dependência no modelo de avaliação, ata de conselho de classe digital, transferência escolar (declaração/guia).
