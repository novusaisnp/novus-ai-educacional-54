...
- **Primeira fatia de "Somar do MVP" da Fase 1 entregue**: Recuperação e Progressão Parcial (ver seção abaixo). Ata de conselho de classe e transferência escolar continuam pendentes dentro do mesmo "somar do MVP".
- **RLS sem política: as 6 tabelas da sessão de 2026-08-04 seguem corrigidas**, e as 3 tabelas novas desta sessão (`academic_terms`, `academic_settings`, `term_results`) já nasceram com RLS + política real na própria migration — não repetir o padrão de "RLS habilitado sem política" nelas.
