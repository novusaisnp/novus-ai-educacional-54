...
- **Descoberta importante de segurança/funcionalidade**: várias tabelas tinham RLS **habilitado sem nenhuma política** — ninguém conseguia ler nem escrever nelas (SELECT retorna vazio silenciosamente, INSERT/UPDATE/DELETE falham com "violates row-level security policy"). `classes`, `enrollments`, `student_guardians` e `documents` **foram corrigidas nesta sessão** (ver Fase 1 — Funil de Admissão e GED do Aluno, abaixo). Ainda faltam `assessments`, `attendance`, `consents`, `grades`, `interactions`, `subjects` — ver Backlog.
- **`OPENAI_API_KEY` não está configurada** nos secrets do projeto (`supabase secrets list` confirmado) — bloqueia testar de ponta a ponta qualquer function de IA nova (ex.: `ai-document-validation`, criada nesta sessão mas não verificada contra a OpenAI de verdade).
...
- **RLS habilitado sem nenhuma política** em `assessments`, `attendance`, `consents`, `grades`, `interactions`, `subjects` — ninguém consegue ler/escrever nessas tabelas hoje no banco real (`classes`/`enrollments`/`student_guardians`/`documents` tinham o mesmo problema, já corrigido nas duas últimas fatias da Fase 1). Descoberto rodando `SELECT c.relname FROM pg_class c WHERE c.relrowsecurity AND NOT EXISTS (SELECT 1 FROM pg_policies p WHERE p.tablename=c.relname)`. Bloqueia silenciosamente qualquer feature que dependa dessas tabelas (frequência, notas, avaliações, LGPD/consents, disciplinas) mesmo que o código do app esteja certo.
...
- Adicionar as políticas RLS que faltam em `assessments`, `attendance`, `consents`, `grades`, `interactions`, `subjects` (mesmo padrão já aplicado em `classes`/`enrollments`/`student_guardians`/`documents`) — hoje essas tabelas são inacessíveis via app.
