...
-- Unique constraints separados
CREATE UNIQUE INDEX prediction_logs_unique_daily 
ON public.prediction_logs (organization_id, model_type, entity_type, entity_id, date_trunc('day', created_at));

CREATE UNIQUE INDEX assessments_feedback_unique 
ON public.assessments_feedback (organization_id, assessment_id, student_id);
...
CREATE OR REPLACE VIEW public.v_risco_evasao AS
SELECT 
  s.id,
...
-- RLS para view
ALTER VIEW public.v_risco_evasao OWNER TO postgres;
GRANT SELECT ON public.v_risco_evasao TO authenticated;
